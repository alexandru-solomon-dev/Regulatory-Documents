При импортировании сторонних ассетов не включайте в проект примеры его использования(как правило выделено в отдельную папку `Example` или `Demo`). Это излишне засоряет проект и ухудшает навигацию.  
При необходимости знакомства или тестирования возможностей ассета создайте отдельный проект.

# Версия Unity

Информация данного раздела может периодически актуализироваться. (Последние изменения: март 2024 года).  
На данным момент используется версия:
- **2022.3.22f1**

Учитывая, что на данный момент версии 2020 и ниже не поддерживаются, рассматривать их не стоит. 
Версии 2021 отличились большим количеством багов и ошибок, которые на билд могут не повлиять, но могут мешают при разработке. Поэтому не рекомендуется выбирать их. Однако, при необходимости рассмотрите версии:
- 2021.3.9f1
- 2021.3.36f1

Последние новости о возможностях LTS версий Unity можно [узнать тут](https://blog.unity.com/topic/lts).
