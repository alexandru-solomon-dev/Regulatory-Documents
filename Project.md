...
В данную директорию помещаются сторонние ассеты c контентом, например из [Unity Asset Store](https://assetstore.unity.com).  
Как правило иерархия папок внутри импортируемого ассета сохранятся.  
Используемые ресурсы из ассета стоит копировать или переносить в папку `Content`.

## Plugins

Сюда помещаются все плагины, фреймворки и функциональные инструменты, если это возможно, например для работы кодогенерации в фреймворке `Entitas`
