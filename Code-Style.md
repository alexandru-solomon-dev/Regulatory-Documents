...
- Документирующие комментарии, [`summary`](#Summary).
...
В случаях, когда приходится использовать сложные алгоритмические решения или какие-либо математические формулы, например определенного физического явления, стоит указать название и по возможности ссылку на источник.
...
Для описания работы методов, области ответственности классов, пояснения для полей, свойств класса и всех общедоступных элементов используются [xml-комментарии](https://learn.microsoft.com/ru-ru/dotnet/csharp/language-reference/xmldoc/).

Summary в обязательном порядке составляются для:  
- классов и структур.
- `public`, `protected` и `internal` методов и эвентов.
- для свойств имеющих `set` с модификатором доступа `public`, `protected` и `internal`, и вызывает какую-либо логику.

> [!Tip]
> `summary` не обязателен для свойств, полей и `private` методов. Однако для некоторых из них рекомендуется писать `summary`, например в случаях если необходимо уточнить работу метода, или указать на что может влиять свойство, или прояснить единицы измерения.