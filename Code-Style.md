...
- Избегайте использования аббревиатур или акронимов в именах, за исключением широко известных и принятых аббревиаций. В случаях их использовании в имени, она считается словом и подвергается соответствующему форматированию. Например, `myJson`, `url` вместо `myJOSN` или `URL`.
...
- Имена классов, методов, перечислений, пространств имен, а также `public`, `protected`, `internal` полей и свойств: **PascalCase**. Например, `MyClass`, `MyMethod()`.
...
- Имена интерфейсов начинаются с префикса заглавной буквы `I`. И как правило имя содержит существительное и/или прилагательного с суффиксом -able.  Например `IMediator`, `IDamageable`, `IPayloadState`.
...
1. Конструкторы
1. Финализаторы
1. Кастомные конструкторы (например для нижекции зависимостей в MonoBehaviour) и методы инициализации
1. Методы
1. `static` методы
...
> - Исключением нарушения порядка расположения элементов в зависимости от уровня доступа является только для достижения расположения сериализивынных полей в логические группы. Чтобы не использовать группировку от сторонних инструментов или Editor скрипты.
...
- В целом, предпочтите передачу класса или структуры, а не `Tuple<T1, .., Tn>`, особенно при возврате большого количества данных.  
- Если было принято решение использовать `Tuple`, используете именованный кортеж для большей ясности.  
...
- Рекомендуется определять новые типы делегатов в случаях, когда методы имеют много(3 и более) входных параметров, особенно если они имеют одинаковые типы данных.
- Поля делегатов которые могут быть `event`, должны быть `event`.
...
> Краткая форма записи невозможно при создании Array без значений, и при инициализации других коллекции со значениями.  
...
## Использование `ref`, `in` и `out`

- Используйте `in`, когда необходимо передать структуру по ссылке без возможности ее изменения.
- Ключевое слово `out` используйте для возврата дополнительных значения и при инициализации параметров внутри метода.
- Поместите параметры с `out` после всех остальных в определении метода, за исключением необязательных параметров и параметра с ключевым словом `params`.
- Старайтесь избегать использования `ref`, рассмотрите вариант использования `in` или `out`. `ref` необходим когда нужно мутировать входные данные. 
- Использовании `ref` при передачи контейнера в метод, подразумевает что предоставленный контейнер может быть заменен на совершенно другой экземпляр контейнера.
...
Несколько атрибутов должны быть каждый в своих скобках и разделены новыми строками. Это позволяет легче добавлять и удалять атрибуты и обеспечивает более удобный поиск.
...
IntelliSense будет автоматически отображать информацию о задокументированых членах. Что может сильно облегчить работу и сэкономит время вам и другим разработчикам.

В большинстве IDE, достаточно ввести 3 слеша (`///`) над членом, который нужно задокументировать, чтобы автоматически сформировался простой шаблон комментария. Если это делается для метода, будет учтена его сигнатура и добавлены соответствующие секции.
...
| `<para>` | Необходим для выделение абзаца. |
...
XML-комментарии в обязательном порядке составлять для:  
- классов, структур, интерфейсов и делегатов.
...
При наследовании класса и реализации интерфейсов следует включать ссылки на все в категории <seealso>.  
Кроме того, нет необходимости повторять комментарии для открытых членов, если они уже присутствуют в родительском типе. За исключением случаев, когда например требуется уточнить работу метода в контексте дочернего класса.  
...
Для xml есть 5 зарезервированных символа, чтобы их отобразить необходимо использовать специальные символьные ссылки:
| Символ | Сущность |
| --- | --- |
| < | `&lt;` |
| > | `&gt;` |
