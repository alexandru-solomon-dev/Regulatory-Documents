...
- `in` используется, когда необходимо передать структуру по ссылке без возможности ее изменения.
- Используйте `out` для возврата дополнительных значения и при инициализации параметров внутри метода.
- `ref` необходим когда нужно мутировать входные данные, сначала рассмотрите вариант использования `in` или `out`. 
- При передачи контейнера с ключевым словом `ref`, подразумевает что предоставленный контейнер может быть заменен на совершенно другой экземпляр контейнера.
