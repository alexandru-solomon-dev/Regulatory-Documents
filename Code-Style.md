...
- Если необходимо часто добавлять и удалять элементы - рассмотрите связанные списки.
  - Если при этом нужен индексатор, то рассмотрите `Dictionary<TKey, TValue>`.  
Словари занимают больше оперативной памяти, чем `List<T>`, однако более быстрое удаление элементов.
  - Если необходима проверка на наличие какого-либо элемента в коллекции - `HashSet<T>`.
- При использовании `List<T>`, `HashSet<T>`, `Dictionary<TKey, TValue>`, при создании укажите ожидаемое количество элементов с небольшим запасом.  
...
Присвоение значения члену возвращаемой структуры не изменяет оригинал - например, невозможно выставить `transform.position.x = 10;` position здесь является свойством, которое возвращает Vector3 по значению.
...
- Если было принято решение использовать `Turple`, используете именованный кортеж. Например, public (float min, float max, int count) GetInfo()`.
